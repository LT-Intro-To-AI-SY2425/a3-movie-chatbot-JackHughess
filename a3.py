...
def title_by_year_range(matches: List[str]) -> List[str]:
    ...
    """
    rList = []
    yearStart = int(matches[0])
    yearEnd = int(matches[1])
    for movie in movie_db:
        if yearStart <= get_year(movie) and yearEnd >= get_year(movie):
            rList.append(get_title(movie))
    return rList
